...
<div class='row-fluid'>
    ...
    <div class="block span8">
        <form action="/manage/index.php/Admin/Cat/up" method="post">
            <div class="block-heading" data-toggle="collapse" data-target="#tablewidget"> 
                学科：<input type='text' style="width: 13%;" name='cat_name' value="<?php echo ($cat["cat_name"]); ?>"/> 
                负责部门：<select name="cat_dep" style="width: 13%;">
                    <option value="<?php echo ($cat["cat_dep"]); ?>"></option>
                ...
                <button id="addmo" class="btn-info" type="button" style="float: right;">添加模块</button></div>
            <div id="tablewidget" class="block-body collapse in">
                <ol id="ol">
                    <?php if(is_array($cat['m'])): $k = 0; $__LIST__ = $cat['m'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($k % 2 );++$k;?><li>
                            模块：
                            <input type="text" name="modu_name[]" value="<?php echo ($m["name"]); ?>"><br/>
                            &nbsp;&nbsp;负责部门：
                            <select class="modu_dep" name="modu_dep[]" style="width: 13%;">
                                <option value="<?php echo ($m["dep"]); ?>"></option>
                                <?php if(is_array($dep)): $k = 0; $__LIST__ = $dep;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dp): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>"><?php echo ($dp); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </li><?php endforeach; endif; else: echo "" ;endif; ?>              
                </ol> 
                <input type="hidden" name="cat_id" value="<?php echo ($cat["cat_id"]); ?>"/>
                <input class="btn-danger" type="submit" value="提交"/>
            </div>
    </div>
</form>
</div>
...
<script>
    $(function () {
        $("#addmo").click(function () {
            var op = $("select[name='cat_dep']").html();
