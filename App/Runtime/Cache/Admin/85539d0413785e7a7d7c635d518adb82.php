...
<div class="span9">
    ...
    <div class="row-fluid">
        ...
        <div class="block span12">
            <div class="block-heading" data-toggle="collapse" data-target="#widget1container">编辑 </div>
            <div id="widget1container" class="block-body collapse in">
                <div class=''>
                     <form action="/manage/index.php/Admin/Course/up" method="post" ecncype='application/x-www-form-urlencoded' >
                        <div class="form-group">
                            <input type="hidden" name='co_id' value="<?php echo ($dat["co_id"]); ?>"/>
                            <label>课程</label>
                            <input type="text" name="co_name" style="width:5%;" class="form-control" value="<?php echo ($dat["co_name"]); ?>"/>&nbsp;&nbsp;&nbsp;&nbsp;
                            <input type="text" name="co_type" style="width:5%;" class="form-control" value="<?php echo ($dat["co_type"]); ?>"/>
                            <label>负责人</label>
                            <select name="co_manager">
                                <option value="<?php echo ($dat["co_manager"]); ?>">
                                    请选择
                                </option>
                                <?php if(is_array($user)): $k = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$u): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>"><?php echo ($u); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        ...
                        <div class="form-group">                  
                            <label>所属学科类别</label>
                            <select name="co_cat">
                                <option value="<?php echo ($dat["co_cat"]); ?>">
                                    请选择学科
                                </option>
                                <?php if(is_array($cat['name'])): $k = 0; $__LIST__ = $cat['name'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($k % 2 );++$k;?><option value="<?php echo ($k); ?>"> <?php echo ($c); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>                   
                        </div>
                        <div class="form-group">
                            <label>开始时间</label>
                            <input type="text" id="start" value="<?php echo ($dat["co_time_s"]); ?>" class="laydate-icon" name="co_time_s" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"/>&nbsp;&nbsp;
                            <input type="text" id="end" value="<?php echo ($dat["co_time_e"]); ?>" class="laydate-icon" name="co_time_e" onClick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" />  
                        </div>
                        <div class="form-group">
                            <label>课程进度</label>
                            <input type="text" name="co_process" style="width: 3%" class="form-control"  value="<?php echo ($dat["co_process"]); ?>"/>%
                        </div>
                        <div class="form-group">
